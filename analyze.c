...
/*
ETHER : netinet/if_ether.h
ARP   : netinet/if_ether.h
IP    : netinet/ip.h
IPv6  : netinet/ip6.h
ICMP  : netinet/ip_icmp.h
ICMPv6: netinet/ip_icmp.h
TCP   : netinet/tcp.h
UDP   : netinet/udp.h
*/
...
int AnalyzeIpv6(u_char *data, int size)
{
    ...
    if (ip6->ip6_nxt == IPPROTO_ICMPV6) {
        ...
        if (checkIP6DATAchecksum(ip6, ptr, len) == 0) {
            fprintf(stderr, "bad icmp6 icmp6 checksum\n");
            return (-1);
        }
...
int AnalyzePacket(u_char *data, int size)
{
    ...
    if (lest < sizeof(struct ether_header)) {
        fprintf(stderr, "lest(%d) < sizeof (struct ehter_header)\n", lest);
        return (-1);
    }
